...
constexpr inline std::array<double, 2> rotate(double x, double y,
                                              double theta) {
	return {
...
constexpr inline double mph2mps(double mph) { return mph / 2.24; }
constexpr inline double mps2mph(double mps) { return mps * 2.24; }
...
struct Car {
	...
	double s;
	double d;
	inline double speed_mps() const { return sqrt(vx * vx + vy * vy); }
	inline int lane() const { return d / 4; }
